...
class Terpene:
    ...
    def validate(self, **kwargs) -> list[ValidationErrorInfo]:
        ...
        if self.subclass not in self.VALID_SUBCLASSES:
            errors.append(ValidationErrorInfo("Terpene.subclass", f"Invalid subclass: {self.subclass}"))
        ...
        if self.precursor and self.precursor not in self.VALID_PRECURSORS:
            errors.append(ValidationErrorInfo("Terpene.precursor", f"Invalid precursor: {self.precursor}"))
    ...
    def from_json(cls, raw: dict[str, Any], **kwargs) -> Self:
        return cls(
            ...
            prenyltransferases=[GeneId.from_json(p, **kwargs) for p in raw.get("prenyltransferases", [])],
