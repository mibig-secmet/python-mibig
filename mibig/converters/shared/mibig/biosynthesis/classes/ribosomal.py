...
class Crosslink:
    ...
    def __init__(self, begin: int, end: int, link_type: str | None, details: str | None, validate: bool = True, **kwargs) -> None:
        self.begin = begin
        self.end = end
    ...
    def validate(self, cds: CDS | None) -> list[ValidationErrorInfo]:
        errors = []

        if self.begin < 0:
            errors.append(ValidationErrorInfo("RippCrosslink.from", "From must be greater than or equal to 0"))
        if self.end < 0:
            errors.append(ValidationErrorInfo("RippCrosslink.to", "To must be greater than or equal to 0"))
        if self.begin >= self.end:
            errors.append(ValidationErrorInfo("RippCrosslink.from", "From must be less than to"))

        if cds:
            if self.begin >= cds.translation_length:
                errors.append(ValidationErrorInfo("RippCrosslink.from", "From must be less than the length of the CDS"))
            if self.end > cds.translation_length:
                errors.append(ValidationErrorInfo("RippCrosslink.to", "To must be less than or equal to the length of the CDS"))
...
class Precursor:
    ...
    def from_json(cls, raw: dict[str, Any], **kwargs) -> Self:
        ...
        if "leader_cleavage_location" in raw:
            leader_cleavage_location = Location.from_json(raw["leader_cleavage_location"], **kwargs)
        if "follower_clavage_location" in raw:
            follower_clavage_location = Location.from_json(raw["follower_clavage_location"], **kwargs)
        return cls(
            gene=gene,
            ...
            crosslinks=[Crosslink.from_json(crosslink, cds=cds) for crosslink in raw.get("crosslinks", [])],
...
class Ribosomal:
    ...
    def validate(self, **kwargs) -> list[ValidationErrorInfo]:
        ...
        if self.subclass not in self.VALID_SUBCLASSES:
            errors.append(ValidationErrorInfo("Ribosomal.subclass", f"Subclass must be one of {', '.join(self.VALID_SUBCLASSES)}"))

        if self.subclass == "RiPP":
            if self.ripp_type not in self.VALID_RIPP_TYPES:
                errors.append(ValidationErrorInfo("Ribosomal.ripp_type", f"Invalid RiPP type: {self.ripp_type}"))
            if self.ripp_type == "other" and not self.details:
                errors.append(ValidationErrorInfo("Ribosomal.details", "Details must be provided for 'other' RiPP types"))
    ...
    def from_json(cls, raw: dict[str, Any], **kwargs) -> Self:
        return cls(
            ...
            precursors=[Precursor.from_json(precursor, **kwargs) for precursor in raw["precursors"]],
            ripp_type=raw.get("ripp_type"),
            details=raw.get("details"),
            peptidases=[GeneId.from_json(peptidase, **kwargs) for peptidase in raw.get("peptidases", [])],
        )
